...
import * as utils from "./utils/taskFunctions.js"
...
const elements = {
  ...
  themeLogo: document.getElementById('logo'), 
...
function filterAndDisplayTasksByBoard(boardName) {
  const tasks = utils.getTasks(); // Fetch tasks from a simulated local storage function
...
function addTask(event) {
  ...
  //Assign user input to the task object
    ...
    const newTask = utils.createNewTask(task);
    if (newTask) {
      addTaskToUI(newTask);
      toggleModal(false);
      elements.filterDiv.style.display = 'none'; // Also hide the filter overlay
...
function toggleTheme() {
  ...
  if (isLightTheme){
    elements.themeLogo.src = "./assets/logo-light.svg"
  }else{
    elements.themeLogo.src = "./assets/logo-dark.svg"
  }
...
function saveTaskChanges(taskId) {
  ...
  const editTask = JSON.parse(localStorage.getItem('tasks')).find(task => task.id === taskId)
  ...
  if (editTask){
    ...
    utils.patchTask(taskId, updatedTask);
  }
  toggleModal(false, elements.editTaskModal);
